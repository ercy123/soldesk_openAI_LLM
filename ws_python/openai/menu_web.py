...
def menu_web_proc():
    ...
    if allowed_size(file_size) == False :
        resp = jsonify({'message' : '파일 사이즈가 25MB 이상입니다. 파일 용량 :' + str(round(file_size/1024/1024)) + ' MB'})
        resp.status_code = 500
        return resp
    if f and allowed_file(f.filename):
        # 업로드 폴더 생성, 절대경로로 생성 (static/uploads)
        upload_folder = os.path.join(os.getcwd(),'static','uploads') # getcwd 는 현재 경로
        ...
        response = client.chat.completions.create(
            ...
            max_tokens=300, # 응답 생성시 최대 1000개의 단어 사용
        )
