...
def movie_proc():
    ...
    data = request.json
    movie = data['movie'] # 0,1,1,1,0 .... 문자열
    
    movie = movie.split(',') # split 의 리턴값은 배열. 콤마(,) 로 구분하여 데이터 저장
    ...
    watch_join = ','.join(watch) # 시청한 영화를 ","로 구분. # join 으로 배열 -> 문자열로 반환
    ...
    response = tool.answer('너는 영화 추천 시스템이야.', prompt, format, llm='gpt-4o-mini-2024-07-18')
